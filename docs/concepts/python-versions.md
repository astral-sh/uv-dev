...
Select it explicitly with a request like `3.13+custom`. Runtime and build variants can be composed,
as in `3.13+freethreaded+custom`. Unqualified requests continue to select the default artifact from
...
The `build_variant` is the single publisher-defined name used after `+` in Python requests. Names
are normalized to lowercase and may contain ASCII letters, digits, and underscores. Setting
`default` to `false` keeps the existing default build selected for an unqualified request like
`3.13`. A catalog should contain only one default for each Python version, runtime variant, and
platform combination. The versioned envelope ensures clients that do not understand build variants
reject the catalog.
...
`UV_PYTHON_BUILD` applies to requests with an explicit build name. For CPython requests without a
build name, such as `3.13` or `3.13+freethreaded`, use
...
`3.13+freethreaded+custom`. The runtime and build name can appear in either order, so
