...
Place `uses: $/.github/actions/disable-github-caches` first in each release job. The self-repository
reference lets GitHub load the action before checkout, and its `pre` hook runs before later action
hooks. The shared binary-build workflow can pass `enabled: ${{ !inputs.allow-cache }}` to preserve
ordinary CI caching.
...
the original resolved service addresses, except from the proxy. Windows provides DNS-based
...
Docker builders, previously issued blob URLs, new service addresses, and separate container
networking are not covered. Keep `ACTIONS_CACHE_MODE: none` and explicit client opt-outs in place.
