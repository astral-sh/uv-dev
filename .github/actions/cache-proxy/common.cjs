...
const content = Buffer.from(
  "Harmless cache-mode probe. No executable content.\n",
);
const version = crypto
  .createHash("sha256")
  .update("uv-cache-proxy-probe-v1")
  .digest("hex");
const prefix = () =>
  `cache-proxy-probe-${process.env.GITHUB_RUN_ID}-${process.env.GITHUB_RUN_ATTEMPT}`;
...
function request(
  url,
  { method = "GET", headers = {}, body, address, ca, timeout = 15000 } = {},
) {
  return new Promise((resolve, reject) => {
    ...
    req.on("error", (error) =>
      reject(
        Object.assign(new Error("Service request failed"), {
...
async function cacheCall(method, data, options = {}) {
  ...
  try {
    ...
    throw new Error(`Cache response was not JSON (${response.status})`);
  }
  return { ...response, data: parsed };
}

const readMetadata = (key, options) =>
  cacheCall(
    "GetCacheEntryDownloadURL",
    { key, version, restore_keys: [] },
    options,
  );
const isDenied = (response, operation) =>
  response.status === 403 &&
  response.headers["x-uv-cache-proxy"] === "denied" &&
  response.data.msg?.startsWith(`cache ${operation} denied:`);
...
async function storage(value, options = {}) {
  ...
  try {
    ...
    throw new Error("Disposable storage request failed");
  }
}
...
async function writeCache(key) {
  ...
  if (!created.data.ok || !url)
    return { http: created.status, denied: false, finalized: false };
  const uploaded = await storage(url, {
    method: "PUT",
    headers: {
  ...
  const finalized = await cacheCall("FinalizeCacheEntryUpload", {
    key,
    version,
  ...
  return {
    http: created.status,
    denied: false,
    finalized: finalized.data.ok === true,
    cacheId: finalized.data.entry_id || finalized.data.entryId || null,
  };
}
