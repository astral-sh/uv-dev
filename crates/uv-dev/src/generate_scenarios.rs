...
fn render_lock_options(output: &mut String, scenario: &Scenario) -> Result<()> {
    ...
    if let Some(resolution) = options.resolution {
        writeln!(output, "        resolution = \"{resolution}\"")?;
    }
    if let Some(fork_strategy) = options.fork_strategy {
        writeln!(output, "        fork-strategy = \"{fork_strategy}\"")?;
    }
...
fn render_resolver_args(
    ...
    if let Some(fork_strategy) = scenario.resolver_options.fork_strategy {
        writeln!(output, "        .arg(\"--fork-strategy={fork_strategy}\")")?;
    }
...
mod tests {
    ...
    use super::{
        ScenarioCase, ScenarioCommand, TemplateKind, check_generated_file, compile_requirements,
        load_scenarios, load_scenarios_from, module_name, render, render_lock_options,
        render_resolver_args, scenarios_for_template,
    };
...
"#
        ...
        .expect("scenario document should parse")
        .scenario()
