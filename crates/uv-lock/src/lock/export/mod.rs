...
/// [`uv_resolver_types::graph_ops::marker_reachability`], but it _also_ attempts to resolve conflict
/// markers. Specifically, in addition to tracking the reachability marker for each node, we also
/// track (for each node) the conditions under which each conflict item is `true`. Then, when
/// evaluating the marker for the node, we inline the conflict marker conditions, thus removing
