...
impl SitePackages {
    /// Create an empty index for an environment using the given interpreter.
            ...
            if parallel {
                ...
                for paths in site_packages.chunks(DIST_INFO_READ_BATCH_SIZE) {
                    ...
                    for (path, dist_info) in paths.iter().zip(dist_infos) {
                        let dist_info = match dist_info {
                            Some(dist_info) => dist_info.and_then(|dist_info| {
                                dist_info.map(InstalledDist::try_from).transpose()
                            }),
