...
pub(super) fn source_file(input: &RequirementsInput, text: impl Into<Arc<str>>) -> SourceFile {
...
impl RequirementsTxtFileError {
    fn diagnostic(&self) -> Option<Diagnostic<'_>> {
        ...
        let diagnostic = match self.error.as_ref() {
            ...
            | RequirementsTxtParserError::RequirementsInput { .. }
            ...
            | RequirementsTxtParserError::MissingRequirementPrefix(_) => return None,
            #[cfg(feature = "http")]
            RequirementsTxtParserError::Reqwest(..)
            | RequirementsTxtParserError::ClientBuild(..) => return None,
        };
        Some(diagnostic)
    }
}
