...
#[tokio::test]
async fn pax_size_controls_extraction_and_alignment() -> Result<()> {
    let payload = [b'x'; 600];
    ...
    for features in [&[][..], &[PreviewFeature::TarCodec][..]] {
        ...
        let _extracted = uv_extract::stream::archive(
            archive.as_slice(),
        ...
        assert_eq!(fs_err::read(target_path.join("payload.bin"))?, payload);
        assert_eq!(fs_err::read(target_path.join("after.txt"))?, b"after");
    }
    Ok(())
}
