...
impl GeneratedScripts {
    pub(super) fn new(
        ...
        if entry_points.is_empty() && scripts.is_empty() {
        ...
        for script in entry_points {
            ...
            let target = format!("{normalized}.cli:main");
            ensure!(
                script_values.insert(script.clone(), target).is_none(),
                "duplicate console-script name `{script}`"
            );
            modules.entry(normalized.to_string()).or_default();
            modules
                .entry(format!("{normalized}.cli"))
                .or_default()
                .insert("main".to_string(), format!("print('Hello from {name}!')"));
        }
        ...
        for (script, target) in scripts {
            validate_script_name(script).map_err(anyhow::Error::msg)?;
                ...
                script_values
                    .insert(script.clone(), target.to_string())
                    .is_none(),
                "duplicate console-script name `{script}`"
            );
        ...
        for (script, target) in &script_values {
