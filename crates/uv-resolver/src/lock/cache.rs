...
impl Cache {
    fn get(&self, contents: &str, skip_wheel_filename_check: bool) -> Option<Lock> {
    ...
    fn insert(&mut self, contents: &str, lock: &Lock, skip_wheel_filename_check: bool) -> bool {
...
pub(super) fn get(contents: &str) -> Option<Lock> {
    ...
    let lock = cache.get(contents, skip_wheel_filename_check);
    drop(cache);
    if lock.is_some()
...
pub(super) fn insert(contents: &str, lock: &Lock) {
    ...
    let inserted = cache
        .lock()
        .is_ok_and(|mut cache| cache.insert(contents, lock, skip_wheel_filename_check));
...
mod tests {
    ...
    #[test]
    fn cache_identity_includes_contents_and_parser_policy() {
        let lock = Lock::from_toml(SOURCE).expect("valid lock");
        let mut cache = Cache::default();
        assert!(cache.insert(SOURCE, &lock, true));
        assert_eq!(cache.get(SOURCE, false), None);
        assert_eq!(cache.get(SOURCE, true), Some(lock.clone()));
        assert_eq!(cache.get(&SOURCE.replace("3.12", "3.13"), true), None);

        assert!(cache.insert(SOURCE, &lock, false));
        assert_eq!(cache.get(SOURCE, false), Some(lock.clone()));
        assert_eq!(cache.get(SOURCE, true), Some(lock));
    }
