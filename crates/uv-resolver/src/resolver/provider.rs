...
/// Return whether a failed metadata request is ignored by its physical index.
fn ignores_metadata_error(
    ...
    index.is_some_and(|index| {
        let physical = index_routes.route_for(index).physical;
        index_locations.ignores_error_code_for(&physical, status)
    })
}
