...
/// Decode an extra name generated by [`encode_conflict_item`].
///
/// User-defined extras may resemble encoded conflict items, so malformed or incomplete encodings
/// are treated as ordinary extras instead of returning an error.
fn decode_conflict_item(raw_extra: &ExtraName) -> Option<ConflictItem> {
    let (kind, tail) = raw_extra.as_str().split_once('-')?;
    if !matches!(kind, "extra" | "group" | "project") {
        return None;
    }
    ...
    match kind {
        "project" if tail.is_empty() => Some(ConflictItem::from(package)),
        "extra" => {
            let extra = tail.strip_prefix('-')?;
            Some(ConflictItem::from((
                package,
                ExtraName::from_str(extra).ok()?,
            )))
        }
        "group" => {
...
/// Resolve unencoded package extra markers and conflict-encoded extra markers in a [`MarkerTree`]
/// based on the conditions under which each conflict item is known to be true.
///
...
/// `scope_package`.
///
/// The activation markers in `known_conflicts` must already be resolved and contain no extra
/// expressions.
pub(crate) fn resolve_activated_extras(
    ...
    const SMALL_EXTRA_LIMIT: usize = 8;

    if marker.is_true() || marker.is_false() {
        return marker;
    }

    // Most dependency markers mention only one or two extras. Keep those on the existing DNF
    // path, avoiding the activation index and repeated BDD restrictions until a marker is wide
    // enough for DNF expansion to dominate.
    let mut small_extras = SmallVec::<[ExtraName; SMALL_EXTRA_LIMIT]>::new();
    let mut extras = FxHashSet::default();
    marker.visit_extras(|_, extra| {
        if !extras.is_empty() {
            extras.insert(extra.clone());
        } else if !small_extras.contains(extra) {
            if small_extras.len() == SMALL_EXTRA_LIMIT {
                extras.extend(small_extras.drain(..));
                extras.insert(extra.clone());
            } else {
                small_extras.push(extra.clone());
            }
        }
    });
    if extras.is_empty() {
        return if small_extras.is_empty() {
            marker
        } else {
            resolve_activated_extras_dnf(marker, scope_package, known_conflicts)
        };
    }

    resolve_activated_extras_bdd(marker, scope_package, known_conflicts, extras)
}

fn resolve_activated_extras_dnf(
    marker: MarkerTree,
    scope_package: Option<&PackageName>,
    known_conflicts: &FxHashMap<ConflictItem, MarkerTree>,
) -> MarkerTree {
    ...
    for clause in marker.to_dnf() {
        ...
        for expression in clause {
            let MarkerExpression::Extra { operator, name } = expression else {
                conjunction = conjunction.and(MarkerTree::expression(expression));
                continue;
            };
            let Some(name) = name.as_extra() else {
                conjunction = conjunction.and(MarkerTree::expression(MarkerExpression::Extra {
                    operator,
                    name,
                }));
                continue;
            };
            let activated = decode_conflict_item(name)
                .and_then(|item| known_conflicts.get(&item).copied())
                .or_else(|| {
                    scope_package.and_then(|package| {
                        let item = ConflictItem::from((package.clone(), name.clone()));
                        known_conflicts.get(&item).copied()
                    })
                })
                .unwrap_or(MarkerTree::FALSE);
            match operator {
                ExtraOperator::Equal => conjunction = conjunction.and(activated),
                ExtraOperator::NotEqual => conjunction = conjunction.and(activated.negate()),
            }
        }
        transformed = transformed.or(conjunction);
    }
    transformed
}

fn resolve_activated_extras_bdd(
    marker: MarkerTree,
    scope_package: Option<&PackageName>,
    known_conflicts: &FxHashMap<ConflictItem, MarkerTree>,
    extras: FxHashSet<ExtraName>,
) -> MarkerTree {
    // Index the encoded conflict items once, instead of encoding and comparing every item for
    // every extra expression in the marker.
    let encoded_conflicts: FxHashMap<_, _> = known_conflicts
        .iter()
        .map(|(item, marker)| (encode_conflict_item(item), *marker))
    ...
    for extra in extras {
        ...
        let activated = encoded_conflicts
            ...
            .or_else(|| {
                scope_package.and_then(|package| {
                    let item = ConflictItem::from((package.clone(), extra.clone()));
                    known_conflicts.get(&item).copied()
                })
            })
            .unwrap_or(MarkerTree::FALSE);
...
mod tests {
    ...
    fn resolve_unencoded_package_extras() {
        ...
        let cm = MarkerTree::from_str("extra == 'foo'").unwrap();
        assert!(resolve_activated_extras(cm, None, &known_conflicts).is_false());
        let other = create_package("other");
        assert!(resolve_activated_extras(cm, Some(&other), &known_conflicts).is_false());
        let cm = MarkerTree::from_str("extra != 'foo'").unwrap();
        assert!(resolve_activated_extras(cm, None, &known_conflicts).is_true());
        assert!(resolve_activated_extras(cm, Some(&other), &known_conflicts).is_true());
    }

    #[test]
    fn resolve_encoded_conflict_kinds() {
        let package = create_package("pkg");
        let other = create_package("other");
        let group = GroupName::from_str("dev").unwrap();
        let known_conflicts = FxHashMap::from_iter([
            (
                ConflictItem::from((package, group)),
                MarkerTree::from_str("sys_platform == 'darwin'").unwrap(),
            ),
            (
                ConflictItem::from(other),
                MarkerTree::from_str("python_version >= '3.12'").unwrap(),
            ),
        ]);

        let marker = MarkerTree::from_str(
            "(extra == 'group-3-pkg-dev' and extra != 'project-5-other') \
             or extra != 'extra-3-pkg-missing'",
        )
        .unwrap();
        assert!(resolve_activated_extras(marker, None, &known_conflicts).is_true());

        let marker =
            MarkerTree::from_str("extra == 'group-3-pkg-dev' and extra != 'project-5-other'")
                .unwrap();
        assert_eq!(
            resolve_activated_extras(marker, None, &known_conflicts),
            MarkerTree::from_str("sys_platform == 'darwin' and python_version < '3.12'").unwrap()
        );
    }

    #[test]
    fn resolve_encoded_extra_precedes_package_scope() {
        let package = create_package("pkg");
        let known_conflicts = FxHashMap::from_iter([
            (
                ConflictItem::from((package.clone(), create_extra("foo"))),
                MarkerTree::from_str("sys_platform == 'darwin'").unwrap(),
            ),
            (
                ConflictItem::from((package.clone(), create_extra("extra-3-pkg-foo"))),
                MarkerTree::from_str("sys_platform == 'win32'").unwrap(),
            ),
        ]);
        let marker = MarkerTree::from_str("extra == 'extra-3-pkg-foo'").unwrap();

        assert_eq!(
            resolve_activated_extras(marker, Some(&package), &known_conflicts),
            MarkerTree::from_str("sys_platform == 'darwin'").unwrap()
        );
    }

    #[test]
    fn decode_generated_conflict_items() {
        let package = create_package("long-pkg-name");
        let items = [
            ConflictItem::from((package.clone(), create_extra("cpu-fast"))),
            ConflictItem::from((package.clone(), GroupName::from_str("dev-tools").unwrap())),
            ConflictItem::from(package),
        ];

        for item in items {
            assert_eq!(
                decode_conflict_item(&encode_conflict_item(&item)),
                Some(item)
            );
        }
        ...
        for name in [
            ...
            assert_eq!(decode_conflict_item(&create_extra(name)), None, "{name}");
        }
    }

    #[test]
    fn resolve_encoded_looking_package_extras() {
        let package = create_package("pkg");
        let darwin = MarkerTree::from_str("sys_platform == 'darwin'").unwrap();
        let python = MarkerTree::from_str("python_version >= '3.12'").unwrap();
        let known_conflicts = FxHashMap::from_iter([
            (
                ConflictItem::from((package.clone(), create_extra("foo"))),
                darwin,
            ...
            (
                ConflictItem::from((package.clone(), create_extra("extra-3-pkg-missing"))),
                python,
            ),
            (
                ConflictItem::from((package.clone(), create_extra("extra-three-pkg-cpu"))),
                python,
            ),
            (
        ...
        for (name, expected) in [
            ...
            let included = MarkerTree::from_str(&format!("extra == '{name}'")).unwrap();
            let excluded = MarkerTree::from_str(&format!("extra != '{name}'")).unwrap();
            assert_eq!(
                resolve_activated_extras(included, Some(&package), &known_conflicts),
                expected,
                "included {name}"
            );
            assert_eq!(
                resolve_activated_extras(excluded, Some(&package), &known_conflicts),
                expected.negate(),
                "excluded {name}"
            );
        }
    ...
    #[test]
    fn resolve_matches_dnf_for_all_extra_combinations() {
        let package = create_package("pkg");
        let other = create_package("other");
        let group = GroupName::from_str("dev").unwrap();
        let known_conflicts = FxHashMap::from_iter([
            ...
            (
                ConflictItem::from((package.clone(), create_extra("local"))),
                MarkerTree::from_str("python_version >= '3.12'").unwrap(),
            ),
            (
                ConflictItem::from((package.clone(), group)),
                MarkerTree::from_str("platform_machine == 'arm64'").unwrap(),
            ),
            (
                ConflictItem::from(other),
                MarkerTree::from_str("implementation_name == 'cpython'").unwrap(),
            ),
            (
                ConflictItem::from((package.clone(), create_extra("extra-3-pkg-foo"))),
                MarkerTree::from_str("sys_platform == 'win32'").unwrap(),
        ...
        for atoms in [
            ["extra-3-pkg-foo", "local", "missing"],
            ["group-3-pkg-dev", "project-5-other", "extra-3-pkg-foo"],
        ] {
            for function in 0..256 {
                ...
                for assignment in 0..8 {
                    ...
                    for (index, atom) in atoms.iter().enumerate() {
                        let operator = if assignment & (1 << index) == 0 {
                            "!="
                        } else {
                            "=="
                        };
                        let expression =
                            MarkerTree::from_str(&format!("extra {operator} '{atom}'")).unwrap();
                        clause = clause.and(expression);
                    }
                    marker = marker.or(clause);
                }
                marker = marker.and(
                    MarkerTree::from_str("python_version >= '3.10' or sys_platform == 'linux'")
                        .unwrap(),
                );
                ...
                assert_eq!(
                    resolve_activated_extras_bdd(marker, Some(&package), &known_conflicts, extras),
                    resolve_activated_extras_dnf(marker, Some(&package), &known_conflicts),
                    "different result for {atoms:?} with function {function:08b}"
                );
            }
        }
    }
    ...
    #[test]
    fn resolve_at_small_marker_threshold() {
        let package = create_package("pkg");
        let darwin = MarkerTree::from_str("sys_platform == 'darwin'").unwrap();
        let python = MarkerTree::from_str("python_version >= '3.12'").unwrap();
        let mut expected = darwin;
        expected = expected.or(python);

        for width in [8, 9] {
            let mut marker = MarkerTree::FALSE;
            let mut known_conflicts = FxHashMap::default();
            for index in 0..width {
                let extra = create_extra(&format!("item-{index}"));
                let item = ConflictItem::from((package.clone(), extra.clone()));
                marker = marker
                    .or(MarkerTree::from_str(&format!("extra == 'extra-3-pkg-{extra}'")).unwrap());
                known_conflicts.insert(item, if index % 2 == 0 { darwin } else { python });
            }

            assert_eq!(
                resolve_activated_extras(marker, None, &known_conflicts),
                expected,
                "different result at width {width}"
            );
        }
    }

    #[test]
    fn resolve_product_of_extra_disjunctions() {
        let package = create_package("pkg");
        let darwin = MarkerTree::from_str("sys_platform == 'darwin'").unwrap();
        let python = MarkerTree::from_str("python_version >= '3.12'").unwrap();
        ...
        for pairs in [4, 5] {
            ...
            for index in 0..pairs {
                ...
                .unwrap();
                marker = marker.and(disjunction);
                known_conflicts.insert(ConflictItem::from((package.clone(), left)), darwin);
                known_conflicts.insert(ConflictItem::from((package.clone(), right)), python);
            }

            assert_eq!(
                resolve_activated_extras(marker, None, &known_conflicts),
                resolve_activated_extras_dnf(marker, None, &known_conflicts),
                "different result for {pairs} disjunctions"
            );
        }
    }
}
