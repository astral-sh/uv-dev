    ...
    for clause in marker.to_dnf() {
        ...
        for expression in clause {
            ...
            let Some(name) = name.as_extra() else {
                conjunction.and(MarkerTree::expression(MarkerExpression::Extra {
                    operator,
                    name,
                }));
                continue;
            ...
            match operator {
                ExtraOperator::Equal => conjunction.and(activated),
                ExtraOperator::NotEqual => conjunction.and(activated.negate()),
            }
        }
...
fn resolve_activated_extras_bdd(
    ...
    for extra in extras {
        ...
        activated_branch.and(activated);
        let mut inactive_branch =
            transformed.simplify_not_extras_with(|candidate| *candidate == extra);
        inactive_branch.and(activated.negate());
        activated_branch.or(inactive_branch);
