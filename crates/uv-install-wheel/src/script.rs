...
impl Script {
    ...
    pub(crate) fn from_value(
        ...
        let captures = regex!(
            r"^(?P<module>[_\p{XID_Start}]\p{XID_Continue}*(?:\.[_\p{XID_Start}]\p{XID_Continue}*)*)\s*:\s*(?P<function>[_\p{XID_Start}]\p{XID_Continue}*(?:\.[_\p{XID_Start}]\p{XID_Continue}*)*)(?:\s*\[\s*(?P<extras>(?:[^,]+,?\s*)+)\])?\s*$"
        )
            .captures(value)
