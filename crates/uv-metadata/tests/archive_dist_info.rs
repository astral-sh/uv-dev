...
#[test]
fn metadata_cardinality_preserves_payload_and_stem() -> Result<(), Box<dyn StdError>> {
    ...
    let (payload, dist_info_stem) = find_archive_dist_info(&filename, files.into_iter())?;
    assert_eq!(payload, 31);
    assert_eq!(dist_info_stem.to_string(), "demo-1.0");
    Ok(())
}
...
fn metadata_search_stops_at_the_first_none() -> Result<(), Box<dyn StdError>> {
    ...
    for (steps, expected) in [
        ...
        assert_eq!(
            find_archive_dist_info(&filename, files)
                .map(|(payload, dist_info_stem)| (payload, dist_info_stem.to_string()))
                .map_err(|error| error.to_string()),
            expected
                .map(|(payload, dist_info_stem)| (payload, dist_info_stem.to_owned()))
