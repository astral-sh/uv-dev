...
pub fn resolve_activated_extras(
    ...
    let mut small_extras: [Option<ExtraName>; SMALL_EXTRA_LIMIT] = std::array::from_fn(|_| None);
    let mut small_extras_len = 0;
    let mut extras = FxHashSet::default();
    marker.visit_extras(|_, extra| {
        if !extras.is_empty() {
            extras.insert(extra.clone());
        } else if !small_extras[..small_extras_len]
            .iter()
            .any(|candidate| candidate.as_ref() == Some(extra))
        {
            if small_extras_len == SMALL_EXTRA_LIMIT {
                extras.extend(small_extras.iter_mut().filter_map(Option::take));
                extras.insert(extra.clone());
            } else {
                small_extras[small_extras_len] = Some(extra.clone());
                small_extras_len += 1;
            }
        }
    });
    if extras.is_empty() {
        return if small_extras_len == 0 {
            marker
        } else {
