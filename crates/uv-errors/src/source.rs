...
impl<'a> SourceSnippet<'a> {
    ...
    pub fn from_span(source: SourceFile, span: Range<usize>) -> Option<Self> {
        ...
        let source = SourceFile::new(source.name, &text[start..end]).with_line_start(line_start);
...
mod tests {
    ...
    #[test]
    fn parser_spans_only_show_the_first_physical_line() {
        let text = "token = 'secret'\r\nitems = [\r\n  'first',\r\n]\r\nother = 'secret'\r\n";
        let start = text.find('[').expect("array in test input");
        let end = text.find(']').expect("array terminator in test input") + 1;
        let snippet = SourceSnippet::from_span(SourceFile::new("pyproject.toml", text), start..end)
