...
mod tests {
    ...
    fn test_cache_control_lookup() -> Result<(), Box<dyn Error>> {
        ...
        let route1 = index_locations.route_for(&url1);
        assert_eq!(
            route1.simple_api_cache_control(),
            Some(HeaderValue::from_static("max-age=300"))
        );
        assert_eq!(
            route1.artifact_cache_control(),
            Some(HeaderValue::from_static("max-age=1800"))
        );

        let url2 = IndexUrl::from_str("https://index2.example.com/simple").unwrap();
        let route2 = index_locations.route_for(&url2);
        assert_eq!(route2.simple_api_cache_control(), None);
        assert_eq!(route2.artifact_cache_control(), None);

        let url3 = IndexUrl::from_str("https://index3.example.com/simple").unwrap();
        let route3 = index_locations.route_for(&url3);
        assert_eq!(route3.simple_api_cache_control(), None);
        assert_eq!(route3.artifact_cache_control(), None);
        Ok(())
    }
    ...
    fn test_pytorch_default_cache_control() -> Result<(), Box<dyn Error>> {
        ...
        let route = index_locations.route_for(&pytorch_url);

        assert_eq!(route.simple_api_cache_control(), None);
        assert_eq!(
    ...
    fn test_nvidia_default_cache_control() -> Result<(), Box<dyn Error>> {
        ...
        let route = index_locations.route_for(&nvidia_url);
