...
impl VariantRequest {
    ...
    pub(crate) const fn build(&self) -> Option<&PythonBuildVariant> {
        self.build.as_ref()
    }
    ...
    /// Return the suffix for display purposes, e.g., `+freethreaded+custom`.
...
mod tests {
    ...
    #[test]
    fn variant_request_matches_build_name() {
    ...
    fn variant_request_from_str() {
        assert_eq!(
            VariantRequest::from_str("freethreaded+custom"),
            Ok(VariantRequest::new(
                PythonVariant::Freethreaded,
                Some(PythonBuildVariant::from_str("custom").unwrap())
            ))
        );
        assert_eq!(
        ...
        assert!(VariantRequest::from_str("custom+custom").is_err());
    }
    ...
    fn python_request_variants() {
        for (request, build) in [
            ...
            assert_eq!(
                ...
                Some(PythonBuildVariant::from_str(build).unwrap()),
                "request: {request}"
            );
        }
    }
