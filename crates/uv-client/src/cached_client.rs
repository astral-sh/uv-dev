...
impl CachedClient {
    ...
    >(
        ...
        let mut retry_state =
            RetryState::start(self.uncached().retry_policy(), packed_req.url().clone());
        self.run_response_callback(
            cache_entry,
            Some(policy),
            start,
        ...
        .await
        .map_err(|err| err.with_retries(retry_state.total_retries()))
    }
...
pub struct DataWithCachePolicy {
    pub data: AlignedVec,
    pub(crate) cache_policy: OwnedArchive<CachePolicy>,
}
