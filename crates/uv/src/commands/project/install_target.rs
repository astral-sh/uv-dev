...
impl<'lock> Installable<'lock> for InstallTarget<'lock> {
    ...
    fn includes_group(
        ...
        if !groups.includes_workspace_groups() {
            // Non-project roots already contribute matching manifest groups for members; retain
            // that behavior outside direct script runs.
            return workspace.is_non_project() && package.is_none();
        }
