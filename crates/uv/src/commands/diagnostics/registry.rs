...
use super::dist_hints;

/// Suggestions owned by one concrete error and any root hidden by its transparent presentation.
///
...
impl<'a> ErrorMetadata<'a> {
    ...
    fn transparent(error: &'a (dyn StdError + 'static)) -> Self {
        Self {
            hints: Hints::none(),
...
pub(super) fn metadata_for_error<'a>(error: &'a (dyn StdError + 'static)) -> ErrorMetadata<'a> {
    ...
    registered!(
        ...
        uv_build_frontend::Error => ErrorMetadata::hinted,
        ...
        uv_lock::PylockTomlError => ErrorMetadata::hinted,
        uv_lock::PylockTomlErrorKind => ErrorMetadata::hinted,
...
fn metadata_for_type<'a, T: StdError + 'static>(
    ...
    if let Some(error) = error.downcast_ref::<T>() {
        ...
        Some(ErrorMetadata::transparent(error.as_ref()))
    } else {
        error
            .downcast_ref::<Arc<T>>()
            .map(|error| ErrorMetadata::transparent(error.as_ref()))
    }
}
...
fn metadata_for_resolve_error(error: &ResolveError) -> ErrorMetadata<'_> {
    ...
    if let ResolveError::Dependencies(_, name, version, chain) = error {
        metadata
            .hints
            .extend(dist_hints(name, Some(version), chain, Hints::none()));
    } else if let ResolveError::Dist(_, dist, chain, _) = error {
        metadata.hints.extend(dist_hints(
            dist.name(),
            dist.version(),
            chain,
            Hints::none(),
