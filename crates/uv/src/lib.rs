...
/// Create the shared HTTP client builder from the resolved global settings.
...
async fn run_with_workspace_cache(
    ...
    // Enable warnings from workspace loading.
    if cli.top_level.global_args.quiet == 0 {
        uv_warnings::enable();
    }

    // Support `UV_WORKING_DIRECTORY` for backward compatibility.
    let directory =
    ...
    // Resolve preview flags before configuration discovery. These flags can change the discovery root.
    ...
    if global_initialization.needs_initialization() {
        // Configure the `tracing` crate for internal logging.
    ...
    //
    // If `--project` points to `pyproject.toml`, use its parent directory.
    // Downstream code such as `FilesystemOptions::find` requires a directory.
    ...
    {
        // Discover the workspace from the target directory, not the current working directory.
        Cow::Owned(std::path::absolute(dir)?)
    } else {
        Cow::Borrowed(&*CWD)
    };

    // Check that an explicit `--project` directory exists.
    // Skip this check for `uv init`, which creates the project directory.
    ...
    if !skip_project_validation {
        if let Some(project_path) = cli.top_level.global_args.project.as_ref() {
            if !project_dir.exists() {
                ...
                // `pyproject.toml` paths already resolve to their parent directory.
                // This error applies only to other file types. See #18508.
                bail!(
    ...
    // Warn about the deprecated `--isolated` argument.
    let deprecated_isolated = if cli.top_level.global_args.isolated {
        match &*cli.command {
            // This command supports its own `--isolated` argument, so do not warn.
            ...
            // `--no-workspace` replaces `--isolated` for this command.
            Commands::Project(command) if matches!(**command, ProjectCommand::Init(_)) => {
                warn_user!(
            ...
            // Ignore `--isolated` for preview APIs and warn.
            ...
            // Continue to support `--isolated` for other APIs and warn.
            _ => {
                warn_user!(
    ...
    // Load filesystem configuration in this priority order:
    // 1. The configuration file from the command line.
    // 2. The nearest `uv.toml` or `pyproject.toml` above the workspace root.
    //    Combine this file with the user configuration file.
    // 3. The nearest `uv.toml` or `pyproject.toml` above the current directory.

    // Pass the cache directory to the initial workspace discovery, even if it does not exist.
    ...
    let filesystem = if let Some(config_file) = cli.top_level.config_file.as_ref() {
        ...
        // Ignore local configuration for user-level commands.
    ...
    let script = if let Some(run_script) = run_script {
        ...
        match &**command {
            // Create a PEP 723 tag for `uv add --script` or `uv lock --script` if needed.
            ...
            // The remaining commands require an existing PEP 723 tag.
    ...
    // Configure the `Printer` for user-facing CLI output.
    let printer = Printer::new(globals.quiet, globals.verbose, globals.no_progress);

    // Configure user-facing CLI warnings.
    if globals.quiet > 0 {
        uv_warnings::disable();
    } else {
        uv_warnings::enable();
    ...
    // Do not initialize the rayon thread pool yet. Initialization makes no-op syncs expensive.
    uv_configuration::RAYON_PARALLELISM.store(globals.concurrency.installs, Ordering::Relaxed);

    // Print resolved settings when requested.
    ...
    // Run this check after the initial workspace discovery resolves cache settings.
    // Run it before any workspace discovery that serves another purpose.
    let cache_dir = std::path::absolute(cache.root())?;
    // PEP 517 hooks can run uv recursively from uv-managed source trees.
    // These trees include source distributions extracted into the cache.
    ...
    // Workspace discovery excludes the cache root.
    // Reuse the earlier discovery only when both caches have the same root.
    ...
    match *cli.command {
        ...
        Commands::Pip(PipNamespace {
            ...
            // Always reinstall source trees from the command line.
            // For example, `uv pip install .` must rebuild and reinstall the current package.
        ...
        Commands::Venv(args) => {
            ...
            // Use `.` as the default prompt because `.venv` is the default environment name.
        ...
        Commands::Tool(ToolNamespace {
            ...
            let (args, invocation_source) = match run_variant {
                ...
                // The outer match excludes all other variants.
                _ => unreachable!(),
            };

            if let Some(shell) = args.generate_shell_completion {
                // Combine `uv tool uvx` with the top-level arguments.
                let mut uvx = Cli::command()
                    ...
                    .unwrap()
                    .clone()
                    // Do not duplicate the top-level `--help` and `--version` flags.
                    .disable_help_flag(true)
                    .disable_version_flag(true);

                // Copy top-level arguments into `uvx`, as in `Args::augment_args`.
                // Skip arguments that conflict with existing `uvx` arguments.
...
fn required_version_error(
    ...
    #[cfg(feature = "self-update")]
    let hint = {
        // Suggest `uv self update` if the minimum required version exceeds the current version.
...
async fn run_project(
    ...
    // Load environment variables that Clap does not handle.
    let environment = EnvironmentOptions::new()?;
    ...
    match *project_command {
        ProjectCommand::Init(args) => {
            ...
            // `uv init` does not support `--project`.
            if explicit_project {
                bail!(
        ...
        ProjectCommand::Lock(args) => {
            ...
            // Use the existing script when available.
            // Otherwise, pass its path so the command can create the script later.
            let script = script
        ...
        ProjectCommand::Add(args) => {
            ...
            // Always reinstall local source trees from the command line.
            for requirement in &requirements {
                let requirement = match requirement {
                    RequirementsSource::Package(requirement) => requirement,
                    RequirementsSource::Editable(requirement) => requirement,
...
/// Suggest `uv pip <subcommand>` when a user enters `uv <subcommand>`.
...
/// # Usage
///
/// Use the uv binary interface as the official public API.
/// Do not call this entry point from external applications.
///
/// uv assumes that it controls the process for its entire lifetime.
/// Calling this entry point more than once in a process can cause unexpected behavior.
///
/// # Safety
///
/// Call this function only when no other threads are running.
#[allow(unsafe_code)]
    ...
    T: Into<OsString> + Clone,
{
    ...
    // Set `UV` to the current executable so child processes inherit it.
    // This includes child processes started by `uv run`.
    if let Ok(current_exe) = std::env::current_exe() {
        // SAFETY: The proof obligation must be satisfied by the caller.
        unsafe {
            // This operation is unsafe in Rust 2024.
            // See https://doc.rust-lang.org/std/env/fn.set_var.html#safety.
            std::env::set_var(EnvVars::UV, current_exe);
        }
    }

    // Parse arguments before passing them to the runtime because `std::env::args` is not `Send`.
    // See https://github.com/rust-lang/rust/pull/48005.
    ...
    // Configure a printer for failures outside command execution.
    // Environment variables can change `no_progress`, but this does not affect important stderr.
    ...
    // Initialize the cache before starting `main2`.
    // Its `papaya` map initializes `seize`, which registers a process-wide memory barrier on Linux.
    // Registration uses an inexpensive kernel fast path while the process has one thread.
    // With multiple threads, registration waits for an RCU (read-copy-update) grace period.
    let workspace_cache = WorkspaceCache::default();

    // See the `min_stack_size` documentation for details about `main2`.
    ...
    let main2 = move || {
        ...
        //
        // The resolver can start HTTP requests that later become unnecessary.
        // Waiting for these requests can delay the CLI exit.
