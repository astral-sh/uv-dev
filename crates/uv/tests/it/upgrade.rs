...
fn upgrade_reports_no_solution_without_mutation() -> Result<()> {
    ...
    error: No solution found when resolving dependencies
      cause: there is no version of idna==9999 and your project depends on idna==9999.
    ");
...
fn upgrade_preserves_hard_constraint_no_solution_failure() -> Result<()> {
    ...
    error: No solution found when resolving dependencies for split (markers: sys_platform != 'linux')
      cause: all versions of foo depend on bar{sys_platform != 'linux'}==2 and your project depends on bar<2, so your project and all versions of foo are incompatible.
             And your project depends on foo==1.0.0.
    "
    );
