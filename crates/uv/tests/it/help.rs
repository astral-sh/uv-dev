...
#[test]
fn index_argument_errors_do_not_repeat_credentials() {
...
#[test]
fn index_argument_errors_keep_the_parse_cause() {
    let context = uv_test::test_context_with_versions!(&[]);

    uv_snapshot!(context.filters(), context.command().args([
        "pip", "compile", "-", "--default-index",
        "https://user:password@example.invalid:bad/simple?sig=signature"
    ]), @"
    exit_code: 2 (failure)
    ----- stderr -----
    error: invalid value '****' for '--default-index <DEFAULT_INDEX>': invalid port number
    ...
    uv_snapshot!(context.filters(), context.command().args([
        "pip", "compile", "-", "--index",
        "https://user:password@example.invalid/simple https://other/name:secret@other.invalid/simple?sig=signature"
    ]), @"
    exit_code: 2 (failure)
    ----- stderr -----
    error: invalid value '****' for '--index <INDEX>': ambiguous user/pass authority in URL (not percent-encoded?): https:***@other.invalid/simple
