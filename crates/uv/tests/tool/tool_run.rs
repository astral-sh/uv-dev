...
#[test]
#[cfg(feature = "test-python-managed")]
fn tool_run_from_install_python_build_name() -> Result<()> {
    ...
    // A compatible unnamed request reuses the installed tool environment.
    uv_snapshot!(context.filters(), context.tool_run()
    ...
    // Running with custom Python leaves the installed unnamed tool environment intact.
    uv_snapshot!(context.filters(), context.tool_run()
        .args(["--from", "./foo", "--python", "3.13", "foo"]), @"
    exit_code: 0 (success)
    ----- stdout -----
...
#[test]
#[cfg(feature = "test-python-managed")]
fn tool_run_from_install_python_build_name_revision() -> Result<()> {
    let context = uv_test::test_context_with_versions!(&[])
        .with_managed_python_dirs()
        .with_tool_dirs()
        .with_filtered_python_keys()
    ...
    context
        .tool_install()
        .args(["--python", "3.12+custom", "./foo"])
        .env(EnvVars::UV_PYTHON_BUILD_REVISION, "20260825")
        .assert()
        .success();

    // A matching revision reuses the installed tool environment.
    uv_snapshot!(context.filters(), context.tool_run()
        .args(["--from", "./foo", "--python", ">=3.12+custom", "foo"])
        .env(EnvVars::UV_PYTHON_BUILD_REVISION, "20260825"), @"
    exit_code: 0 (success)
    ----- stdout -----
    ...
    // Both Pythons satisfy the version and build-name request, but only 3.13 has this revision.
    uv_snapshot!(context.filters(), context.tool_run()
        .args(["--from", "./foo", "--python", ">=3.12+custom", "foo"])
    ...
    // Running with another revision leaves the installed tool available for compatible requests.
    uv_snapshot!(context.filters(), context.tool_run()
        .args(["--from", "./foo", "--python", ">=3.12+custom", "foo"])
        .env(EnvVars::UV_PYTHON_BUILD_REVISION, "20260825"), @"
    exit_code: 0 (success)
...
#[test]
#[cfg(feature = "test-python-managed")]
fn tool_run_reresolve_python_build_name() -> Result<()> {
    let context = uv_test::test_context_with_versions!(&[])
        .with_managed_python_dirs()
        .with_tool_dirs()
        .with_filtered_python_keys()
    ...
    for minor in [11, 12] {
        ...
        // Prefer custom 3.11 initially, then unnamed 3.12 before custom 3.12 during refinement.
        if minor == 12 {
...
#[test]
#[cfg(feature = "test-python-managed")]
fn tool_run_reresolve_python_build_name_revision() -> Result<()> {
    let context = uv_test::test_context_with_versions!(&[])
        .with_managed_python_dirs()
        .with_tool_dirs()
        .with_filtered_python_keys()
    ...
    let context = context
        ...
        .with_env(EnvVars::UV_PYTHON_PREFERENCE, "system")
        .with_env(EnvVars::UV_PYTHON_BUILD_REVISION, "20260901");
    ...
    // Start with 3.11 so the tool's Python requirement triggers refinement.
    uv_snapshot!(context.filters(), context.python_find()
        .args([">=3.11+custom", "--show-version"]), @"
    exit_code: 0 (success)
    ----- stdout -----
    ...
    // Refinement must replace the installed 3.12 revision with the requested download.
    uv_snapshot!(context.filters(), context.tool_run()
        .args(["--from", "./foo", "--python", ">=3.11+custom", "foo"]), @"
    exit_code: 0 (success)
    ----- stdout -----
