...
fn tool_upgrade_with() {
    ...
    hint: To upgrade `python-dotenv` from the exact pin `0.10.2.post2`, repeat its original `uv tool install` command with a relaxed or updated version pin, keeping the other installation options.
    ");
}
...
async fn tool_upgrade_resolution_hints() -> Result<()> {
    ...
    error: Failed to upgrade simple-launcher
      ...
      info: An index URL (http://[LOCALHOST]/simple) could not be queried due to a lack of valid authentication credentials (401 Unauthorized)
    ");
