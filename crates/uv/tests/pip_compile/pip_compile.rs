...
/// Apply conditional build policies before filtering generated hashes.
#[test]
fn generate_hashes_build_policy_if_necessary_preview() -> Result<()> {
    let context = uv_test::test_context!("3.12");
    context
        .temp_dir
        .child("requirements.in")
    ...
    ----- stdout -----
    # This file was autogenerated by uv via the following command:
    #    uv pip compile --cache-dir [CACHE_DIR] requirements.in --build-policy if-necessary --generate-hashes
    iniconfig==2.0.0 \
        --hash=sha256:b6a85871a79d2e3b22d2d1b94ac2824226a63c6b741c88f7ae975f18b6778374
        # via -r requirements.in
...
fn build_policy_unnamed_source_metadata() -> Result<()> {
    ...
    let output = context
        ...
        .arg("allow")
        .arg("--build-policy-package")
        .arg("review-pkg=disallow")
        .env(EnvVars::UV_PREVIEW_FEATURES, "build-policy")
        .output()?;
    assert!(!output.status.success(), "{output:?}");
    assert!(
        String::from_utf8_lossy(&output.stderr).contains(
            "Building source distributions is disabled, but attempted to build `review-pkg`"
        ),
        "{output:?}"
    );
    assert!(project.child("import-ran").exists());
