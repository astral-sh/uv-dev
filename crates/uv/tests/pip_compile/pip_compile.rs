...
#[tokio::test]
async fn compile_fallback_interpreter_system_only_catalog() -> Result<()> {
    let context = uv_test::test_context!("3.12");
    ...
    for catalog in [
        ...
        // System-only fallback must not depend on a remote or local download catalog.
        allow_duplicates! {
            uv_snapshot!(context.filters(), context.pip_compile()
                .arg("requirements.in")
                .args(["--python-version", "3.8", "--python-preference", "only-system", "--no-python-downloads"])
                .env(EnvVars::UV_PYTHON_DOWNLOADS_JSON_URL, catalog)
                .env(EnvVars::UV_HTTP_RETRIES, "0"), @"
            exit_code: 0 (success)
            ----- stdout -----
            # This file was autogenerated by uv via the following command:
            #    uv pip compile --cache-dir [CACHE_DIR] requirements.in --python-version 3.8 --python-preference only-system --no-python-downloads
