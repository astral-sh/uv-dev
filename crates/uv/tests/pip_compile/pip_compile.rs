...
fn include_build_dependencies_preserves_artifact_hashes() -> Result<()> {
    ...
    ----- stderr -----
    warning: The `--include-build-dependencies` option is experimental and may change without warning. Pass `--preview-features pip-build-dependencies` to disable this warning.
    Resolved 3 packages in [TIME]
    ");
...
/// Include the complete build dependency closure without changing artifact selection.
#[test]
fn include_build_dependencies_source_distribution() -> Result<()> {
    ...
    uv_snapshot!(context.filters(), context.pip_compile()
        ...
        .arg("--preview-features").arg("pip-build-dependencies"), @r"
    exit_code: 0 (success)
    ----- stdout -----
    # This file was autogenerated by uv via the following command:
    #    uv pip compile --cache-dir [CACHE_DIR] requirements.in --include-build-dependencies -o requirements.txt --preview-features pip-build-dependencies
    editables==0.5
        # via hatchling
    hatchling==1.20.0
        # via source-only
...
/// Remove build requirements when resolution switches away from the source distribution that
/// declared them.
#[test]
fn include_build_dependencies_retracts_rejected_source_requirements() -> Result<()> {
    let context = uv_test::test_context!("3.12");
    let mut scenario = Scenario::empty();
    scenario.packages.insert(
        PackageName::from_str("chooser")?,
        Package {
            versions: BTreeMap::from([
                (
                    Version::from_str("1.0.0")?,
                    PackageMetadata {
                        requires: vec![Requirement::from_str("shared==3.0.0")?],
                        sdist: false,
                        wheel: true,
                        ..PackageMetadata::default()
                    },
                ...
                (
                    Version::from_str("2.0.0")?,
                    PackageMetadata {
                        requires: vec![Requirement::from_str("shared==2.0.0")?],
                        extra_build_requires: vec![Requirement::from_str("helper==1.0.0")?],
                        sdist: true,
                        wheel: false,
                        ..PackageMetadata::default()
                    },
    ...
    scenario.packages.insert(
        ...
        Package {
            versions: BTreeMap::from([(
                Version::from_str("1.0.0")?,
                PackageMetadata {
                    requires: vec![Requirement::from_str("shared==1.0.0")?],
                    sdist: false,
                    wheel: true,
                    ..PackageMetadata::default()
                },
    ...
    uv_snapshot!(context.filters(), context.pip_compile()
        ...
        .arg("--index-url").arg(server.index_url())
        .arg("--include-build-dependencies")
        .env(EnvVars::UV_PREVIEW_FEATURES, "pip-build-dependencies"), @r"
    exit_code: 0 (success)
    ----- stdout -----
    # This file was autogenerated by uv via the following command:
...
/// Preserve the build resolution that was used to discover dynamic backend requirements.
#[test]
fn include_build_dependencies_preserves_build_resolution() -> Result<()> {
    let context = uv_test::test_context!("3.12");
    let mut scenario = Scenario::empty();
    scenario.packages.insert(
    ...
    for (name, version) in [("bar", "1.0.0"), ("foo", "2.0.0")] {
        scenario.packages.insert(
    ...
    project.child("pyproject.toml").write_str(indoc! {r#"
        [build-system]
        requires = ["flit-core==3.9.0", "build-helper>=1"]
        ...
        name = "local-project"
        version = "1.0.0"
        description = "A package with version-dependent dynamic build requirements"
    "#})?;
    project.child("backend.py").write_str(indoc! {r#"
        from build_helper import __version__
    ...
    uv_snapshot!(context.filters(), context.pip_compile()
        ...
        .arg("--index-url").arg(server.index_url())
        .arg("--include-build-dependencies")
        .arg("--resolution").arg("lowest")
        .env(EnvVars::UV_PREVIEW_FEATURES, "pip-build-dependencies"), @r"
    exit_code: 0 (success)
    ----- stdout -----
    # This file was autogenerated by uv via the following command:
    #    uv pip compile --cache-dir [CACHE_DIR] requirements.in --include-build-dependencies --resolution lowest
    build-helper==2.0.0
        # via local-project
    flit-core==3.9.0
        # via local-project
...
fn include_build_dependencies_dynamic_hook_cached() -> Result<()> {
    ...
    context.temp_dir.child("uv.toml").write_str(indoc! {r#"
        preview-features = ["pip-build-dependencies"]
    ...
    ----- stdout -----
    # This file was autogenerated by uv via the following command:
    #    uv pip compile --cache-dir [CACHE_DIR] requirements.in --no-include-build-dependencies
    ./project
        # via -r requirements.in
