...
#[test]
fn compile_lowest_extra_with_lower_bound_many_dependencies() -> Result<()> {
    let context = uv_test::test_context!("3.12");
    ...
    requirements_in.write_str(&format!(
        "{}\na>=1\nb\nc==1\n",
        ["A[extra_b,extra_c] ; python_full_version >= '3.12'"; 32].join("\n")
    ))?;
    ...
    ----- stderr -----
    warning: The direct dependency `b` is unpinned. Consider setting a lower bound when using `--resolution lowest` or `--resolution lowest-direct` to avoid using outdated versions.
    Resolved 3 packages in [TIME]
    ");
