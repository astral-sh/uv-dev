...
/// Detect the package name from an unnamed HTTP requirement.
#[test]
fn unnamed_http_requirement() -> Result<()> {
    // The filename does not contain the package name, so we need to download the archive.
    let context = uv_test::test_context!("3.12");
    let archive = context.temp_dir.child("1.0.0.tar.gz");
    write_tar_gz(
        File::create(archive.path())?,
        &[(
            "archive-1.0.0/pyproject.toml",
            indoc! {r#"
                [project]
                name = "example"
                version = "1.0.0"
    ...
    let requirements_in = context.temp_dir.child("requirements.in");
    requirements_in.write_str(&format!("{}/1.0.0.tar.gz", server.url()))?;
    ...
    example @ http://[LOCALHOST]/1.0.0.tar.gz
        # via -r requirements.in
