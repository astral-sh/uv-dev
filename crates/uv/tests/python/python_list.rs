...
async fn python_list_remote_ndjson_metadata_refreshes_changed_etag() -> Result<()> {
    ...
    let metadata = |version: &str| {
        serde_json::json!({
            "version": format!("{version}+20250101"),
            "artifacts": [{
                "platform": platform,
                "variant": "install_only",
                "url": "https://example.com/cpython.tar.gz",
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(
            ResponseTemplate::new(200)
                ...
                .set_body_raw(metadata("3.13.8").to_string(), "application/x-ndjson"),
        )
        .up_to_n_times(1)
        .mount(&server)
        .await;
    ...
    exit_code: 0 (success)
    ----- stdout -----
    cpython-3.13.8-[PLATFORM]    <download available>
    ");
...
/// Reuse cached Python metadata when the remote manifest keeps the same `ETag`.
#[tokio::test]
async fn python_list_remote_ndjson_metadata_uses_cache() -> Result<()> {
    let context = uv_test::test_context_with_versions!(&[])
        .with_filtered_python_keys()
        .with_filtered_latest_python_versions();
    let server = MockServer::start().await;
