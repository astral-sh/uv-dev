...
fn invert_preserves_marker_split_versions() -> Result<()> {
    ...
    context.temp_dir.child("uv.lock").write_str(
        ...
        wheels = [
            { url = "https://example.com/bar-1.0.0-py3-none-any.whl" },
        ]
        dependencies = [
...
/// Root dependency groups can reference the same split package through multiple marker-specific
/// requirements. Every matching locked version must remain reachable when the fork-marker unions
/// are reused.
#[cfg(feature = "test-universal")]
#[test]
fn non_project_repeated_group_fork_markers() -> Result<()> {
    let context = uv_test::test_context!("3.12");
    ...
    context.temp_dir.child("pyproject.toml").write_str(
        ...
        dev = [
            ...
            "bar ; sys_platform != 'win32' and sys_platform != 'linux'",
        ]
        test = [
            "bar ; sys_platform == 'win32'",
            "bar ; sys_platform != 'win32'",
    ...
    context.temp_dir.child("uv.lock").write_str(
        ...
        dev = [
            { name = "bar", marker = "sys_platform == 'win32'" },
            { name = "bar", marker = "sys_platform == 'linux'" },
            { name = "bar", marker = "sys_platform != 'win32' and sys_platform != 'linux'" },
        ]
        test = [
            { name = "bar", marker = "sys_platform == 'win32'" },
            { name = "bar", marker = "sys_platform != 'win32'" },
    ...
    uv_snapshot!(context.filters(), context.tree().arg("--frozen").arg("--universal").arg("--all-groups"), @"
    exit_code: 0 (success)
    ----- stdout -----
    bar v1.0.0 (group: test)
    bar v2.0.0 (group: test)
    bar v1.0.0 (group: dev)
    bar v2.0.0 (group: dev)
    bar v2.0.0 (group: dev)
    ");

    uv_snapshot!(context.filters(), context.export().arg("--frozen").arg("--all-groups").arg("--no-hashes"), @"
    exit_code: 0 (success)
    ----- stdout -----
    # This file was autogenerated by uv via the following command:
    #    uv export --cache-dir [CACHE_DIR] --frozen --all-groups --no-hashes
    bar==1.0.0 ; sys_platform == 'win32'
    bar==2.0.0 ; sys_platform != 'win32'
    ");
    ...
    let workspace_id = metadata["workspace"]["id"]
        .as_str()
        .ok_or_else(|| anyhow::anyhow!("workspace ID was not a string"))?;
    let dev = resolution
        .get(&format!("{workspace_id}:dev"))
        .ok_or_else(|| anyhow::anyhow!("missing dev group"))?;
    let test = resolution
        .get(&format!("{workspace_id}:test"))
        .ok_or_else(|| anyhow::anyhow!("missing test group"))?;
    ...
    }), @r#"
    {
      "dev": [
        {
          "id": "bar==2.0.0@registry+https://pypi.org/simple",
        ...
        {
          "id": "bar==1.0.0@registry+https://pypi.org/simple",
          "marker": "sys_platform == 'win32'"
        }
      ],
      "test": [
