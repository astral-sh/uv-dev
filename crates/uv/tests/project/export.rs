...
#[cfg(all(feature = "test-universal", feature = "test-git"))]
#[test]
fn pep_751_git_sha_named_branch() -> Result<()> {
    let context = uv_test::test_context!("3.12");
    ...
    context
        ...
        .write_str(&formatdoc! {r#"
            ...
            [tool.uv.sources]
            example = {{ git = "{repository_url}", branch = "{branch}" }}
        "#})?;
    ...
    uv_snapshot!(filters, context.export()
        .arg("--format")
        .arg("pylock.toml")
        .arg("--output-file")
        .arg("pylock.toml")
        .arg("--no-emit-project"), @r#"
    exit_code: 0 (success)
    ----- stdout -----
    # This file was autogenerated by uv via the following command:
    #    uv export --cache-dir [CACHE_DIR] --format pylock.toml --output-file pylock.toml --no-emit-project
    lock-version = "1.0"
    created-by = "uv"
    requires-python = ">=3.12"
    ...
    name = "example"
    version = "0.1.0"
    vcs = { type = "git", url = "file://[TEMP_DIR]/repository", requested-revision = "refs/heads/[COMMIT]", commit-id = "[COMMIT]" }
