...
// `parent` requires `child>=2.5,<4`. Force-installing `child==2.4` makes the
// installed environment incompatible without changing its other dependencies.
#[test]
...
// Force-install versions below both dependency bounds to report multiple
// incompatibilities in the same environment.
#[test]
