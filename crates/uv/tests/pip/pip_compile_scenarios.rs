...
fn compatible_python_incompatible_override() -> Result<()> {
    ...
    error: No solution found when resolving dependencies
      ...
      info: The `--python-version` value (>=3.9) includes Python versions that are not supported by your dependencies (e.g., a==1.0.0 only supports >=3.10)
...
fn python_patch_override_no_patch() -> Result<()> {
    ...
    error: No solution found when resolving dependencies
      cause: Because the requested Python version (>=3.9) does not satisfy Python>=3.9.4 and a==1.0.0 depends on Python>=3.9.4, we can conclude that a==1.0.0 cannot be used.
             And because you require a==1.0.0, we can conclude that your requirements are unsatisfiable.
      info: The `--python-version` value (>=3.9) includes Python versions that are not supported by your dependencies (e.g., a==1.0.0 only supports >=3.9.4)
