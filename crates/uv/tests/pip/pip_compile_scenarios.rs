...
fn compatible_python_incompatible_override() -> Result<()> {
    ...
    error: No solution found when resolving dependencies
      cause: the requested Python version (>=3.9) does not satisfy Python>=3.10 and a==1.0.0 depends on Python>=3.10, so a==1.0.0 cannot be used.
             And you require a==1.0.0.
...
fn python_patch_override_no_patch() -> Result<()> {
    ...
    ----- stderr -----
    error: No solution found when resolving dependencies
      cause: the requested Python version (>=3.9) does not satisfy Python>=3.9.4 and a==1.0.0 depends on Python>=3.9.4, so a==1.0.0 cannot be used.
             And you require a==1.0.0.
