...
fn conflict_in_fork() -> Result<()> {
    ...
    error: No solution found when resolving dependencies for split (markers: sys_platform == 'os2')
      cause: all versions of c depend on d==2 and all versions of b depend on d==1, so all versions of b and all versions of c are incompatible.
             And a<=1.0.0 depends on b, so a<=1.0.0 and all versions of c are incompatible.
             And a<=1.0.0 depends on c and your project depends on a{sys_platform == 'os2'}<2.
...
fn fork_conflict_unsatisfiable() -> Result<()> {
    ...
    error: No solution found when resolving dependencies
      cause: your project depends on a>=2 and a<2.
    "
    );
...
fn fork_marker_disjoint() -> Result<()> {
    ...
    ----- stderr -----
    error: No solution found when resolving dependencies
      cause: your project depends on a{sys_platform == 'linux'}>=2 and a{sys_platform == 'linux'}<2.
    "
    );
...
fn fork_non_local_fork_marker_direct() -> Result<()> {
    ...
    ----- stderr -----
    error: No solution found when resolving dependencies
      cause: all versions of b depend on c>=2.0.0 and all versions of a depend on c<2.0.0, so all versions of a and all versions of b are incompatible.
             And your project depends on a{sys_platform == 'linux'}==1.0.0 and b{sys_platform == 'darwin'}==1.0.0.
    "
    );
...
fn fork_non_local_fork_marker_transitive() -> Result<()> {
    ...
    ----- stderr -----
    error: No solution found when resolving dependencies
      cause: all versions of b depend on c{sys_platform == 'darwin'}>=2.0.0 and all versions of a depend on c{sys_platform == 'linux'}<2.0.0, so all versions of a and all versions of b are incompatible.
             And your project depends on a==1.0.0 and b==1.0.0.
    "
    );
