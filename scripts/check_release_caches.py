...
The shared binary build may enable caches in CI, but release callers must opt out.
...
def check_release_caches(root: Path) -> tuple[list[str], int]:
    ...
    def check_cache_mode(
        ...
        *,
        required: bool = False,
        cacheable_build: bool = False,
    ) -> None:
        environment = settings.get("env", {})
        mode = environment.get("ACTIONS_CACHE_MODE")
        if (
            (required or "ACTIONS_CACHE_MODE" in environment)
            and mode != "none"
            and not (cacheable_build and mode == BUILD_CACHE_MODE)
        ):
    ...
    def check_local(
        ...
        *,
        ...
        if (
            workflow
            and path == root / CACHEABLE_BUILD_WORKFLOW
            and (inputs or {}).get("allow-cache") != "false"
        ):
            errors.append(f"{location}: release builds must set allow-cache to 'false'")
        if not workflow:
            path = next(
                (
                    path / name
    ...
    def check_step(
        step: dict[str, Any], location: str, *, cacheable_build: bool = False
    ) -> None:
        ...
        if action in DISABLED_CACHE_INPUTS:
            ...
            if inputs.get(name) != value and not (
                cacheable_build
                and action == "astral-sh/setup-uv"
                and inputs.get(name) == BUILD_UV_CACHE
            ):
    ...
    while pending:
        ...
        if workflow:
            cacheable_build = path == root / CACHEABLE_BUILD_WORKFLOW
            if cacheable_build:
                cache_input = (
                    document.get("on", {})
                    .get("workflow_call", {})
                ...
                if (
                    cache_input.get("type") != "boolean"
                    or cache_input.get("default") != "false"
                ):
                    errors.append(
                        f"{location}: allow-cache must be a boolean input with default 'false'"
                    )
            # A caller's workflow environment is not inherited by a reusable workflow.
            check_cache_mode(
                document, location, required=True, cacheable_build=cacheable_build
            )
            for name, job in document.get("jobs", {}).items():
                ...
                if uses := job.get("uses"):
                    if uses.startswith(("./", "$/")):
                        check_local(
                            uses, job_location, workflow=True, inputs=job.get("with")
                        )
                    else:
                        errors.append(
                ...
                for index, step in enumerate(job.get("steps", [])):
                    check_step(
                        step,
                        f"{job_location}.steps[{index}]",
                        cacheable_build=cacheable_build,
                    )
        else:
