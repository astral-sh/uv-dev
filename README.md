...
The strict behavior is established rather than accidental. astral-sh/uv#20784 changed the shared discovery path so `uv check` ignores malformed discovered metadata, but explicitly kept `uv workspace list --scripts` strict and added an integration test for the distinction. The closest history explains the current behavior and the exact duplicate-block error, but does not duplicate this request.
...
William Woodruff subsequently opened astral-sh/uv#21416 to implement that decision. The pull request is open and links this issue for closure.
...
astral-sh/uv#21416 is an open implementation pull request. Its verified scope is to warn and skip only PEP 723 parse failures during `uv workspace list --scripts`; workspace traversal, file-read, and other discovery errors remain hard failures. It updates `workspace_list_scripts_invalid_metadata` to cover successful listing with the malformed candidate skipped.
...
- astral-sh/uv#21416 — “Skip invalid scripts in `uv workspace list --scripts`” (open pull request). This is the direct implementation for this issue. It warns and skips PEP 723 parse failures, preserves hard failures for traversal and file-read errors, and updates the existing integration test.
