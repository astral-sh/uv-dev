...
`west packages pip` does not return a consolidated dependency set. It emits pip arguments containing multiple `-r` requirement-file paths drawn from Zephyr, MCUboot, and workspace modules. The workflow also has a bootstrap dependency: `west` must be installed before `west packages pip` can be invoked to discover those later inputs, so the reporter says at least two installation phases are required. The generated files collectively contain hundreds of requirements and constraints, making independent duplication and maintenance in a project `pyproject.toml` impractical for them.
...
A second reporter supplies a metadata-explicit reproduction on uv 0.12.15 for Windows x86-64: create two local packages that require `flake8==7.2.0` and `flake8==7.3.0`, respectively, then install them into the same environment with separate `uv pip install` calls. Both local packages remain installed and Flake8 ends at 7.3.0, so the first package's exact declared dependency is no longer satisfied. A maintainer confirmed that `uv pip check` detects this state and reports that `blab-1` requires 7.2.0 while 7.3.0 is installed. This directly demonstrates the installed-package incompatibility variant and confirms the separate validation mechanism without requiring persisted command-line history.
...
The second reporter ran `uv check` and received `No python files found` followed by `All checks passed`. That command checks project source and does not validate the installed dependency graph. Maintainers ran the relevant `uv pip check` command against the reproduction; it checked six packages, found one incompatibility, and reported that `blab-1` requires `flake8==7.2.0` but 7.3.0 is installed. The remaining request is therefore about surfacing or preventing that known incompatibility during `uv pip install`, not a failure of the environment-checking command.
...
For any narrower `uv pip` change, one maintainer considers emitting a warning or error when a requested package is incompatible with another package already in the environment much more plausible than retaining requirements from previous invocations. This is a possible diagnostic improvement, not agreement to add stateful resolution semantics.
...
For the West example, the immediate technical investigation is to inspect the dependency metadata shipped by `west==1.5.0` and determine whether it excludes `pyyaml==3.10`. That reproduction should also be completed with the exact failing `west` command and traceback, plus the uv version and platform. The local-package example now confirms that installed dependency conflicts are observable through `uv pip check`, matching the maintainer's preferred warning-or-error approach. The remaining design questions are whether install diagnostics should be automatic or opt-in, whether an error should occur before mutation, and how this would relate to existing `--strict` behavior.
...
The maintainer direction indicates that the proposed history-preserving flag conflicts with the intended role of `uv pip install`; `uv sync` is the supported stateful alternative. A narrower `uv pip` enhancement that warns or errors on incompatibility with installed package metadata remains plausible. This does not change the classification: the requested behavior is still a new capability rather than a defect in the documented installation model.
