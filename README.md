...
Thanks, this is a bug. The `--locked` error is expected because changing a static project version makes the lockfile out of date, but `uv version` should not retain its `pyproject.toml` edit after that validation fails. The current implementation writes the file before `lock_and_sync` and propagates the error without restoring it.
