...
The maintainer's direction lowers the likelihood of the proposed history-preserving flag but leaves open a narrower `uv pip` enhancement that warns or errors on incompatibility with installed package metadata. This does not change the classification: either form is a new capability.
